...
class CharDecoder(nn.Module):
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        loss_func = nn.CrossEntropyLoss(ignore_index=self.pad_token, reduction='sum')
        s_t, (last_hidden, last_cell) = self.forward(char_sequence[:-1], dec_hidden)
        s_t = s_t.permute(1,2,0)
        loss = loss_func(s_t, char_sequence[1:].t())
        return loss
        ### END YOUR CODE
