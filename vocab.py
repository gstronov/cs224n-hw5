...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        if type(sents[0]) is list:
            return [[[self.start_of_word]+[self[c] for c in w] + [self.end_of_word] for w in s] for s in sents]
        return [[self.start_of_word]+[self[c] for c in w] + [self.end_of_word] for w in sents]
        ### END YOUR CODE
